...
v3.0 additions:
  - Bedroom protection: requires recent motion before playing bedroom audio
  - Echo device protection: blocks Echo entities (triggers TV)
  - Volume levels corrected (were 0.10-0.16, now 0.22-0.28)
  - media_content_type fixed to 'spotify://playlist'
  - Confirmed playlist URIs; new kids/dinner/evening playlists added
  - Kids mode volume capped at 0.30
...
ECHO_ENTITIES = [
    'media_player.chatsworth_living_room_echo_show',
...
LIB = {
    ...
    'evening': [
        ...
        {'id':'37i9dQZF1DXcKnb4wcRKrO','name':'Chill Evening','vibe':'relaxed'},
    ],
    'night': [
    ...
    'kids':       [
        {'id':'37i9dQZF1DX6aTaZa0K6VA','name':'Disney Hits'},
        {'id':'37i9dQZF1DWVlYsZJXBFMo','name':'Kids Pop'},
        {'id':'37i9dQZF1DX2M1RktxUUHE','name':'Family Road Trip'},
        {'id':'37i9dQZF1DXa8NOEUWPn9W','name':'Happy Hits'},
        {'id':'7LD17YaJftpf0WMg40h25L','name':'Kids Dance Party Clean'},
        {'id':'2k1TzwejfDMu9vszNPQE4s','name':'Kids Dance Party Fun'},
        {'id':'1P27ra5VqAizmkcUzVAvp2','name':'Kids Party Songs 2026'},
    ],
    'dinner':     [
        {'id':'37i9dQZF1DX4xuWVBs4FgJ','name':'Dinner Jazz'},
        {'id':'37i9dQZF1DWVqJMsgEN0F4','name':'Acoustic Evening'},
        {'id':'37i9dQZF1DWWKeNBqaIy5U','name':'Dinner Jazz Classics'},
    ],
...
# v3.0: Bedroom safety check
def is_bedroom_safe():
    """Check if bedroom has recent motion — required before any bedroom audio."""
    try:
...
def play_sonos(pid, vol=None):
    ...
    if SONOS in BEDROOM_ENTITIES and not is_bedroom_safe():
        logger.warning(f'play_sonos: Skipping bedroom speaker {SONOS} — no recent bedroom motion')
        return
    ...
    time.sleep(1)
    if vol is None:
        # v3.0: Corrected volume levels (were 0.10-0.16, far too quiet)
        vol = {'morning_early':0.22,'morning_late':0.25,'afternoon':0.28,'evening':0.25,'night':0.15}.get(time_slot(), 0.25)
        if kids_mode:
            vol = min(vol + 0.03, 0.30)  # v3.0: kids vol cap at 0.30
    http.post(f'{HA_URL}/api/services/media_player/volume_set', headers=hh, json={'entity_id':SONOS,'volume_level':vol}, timeout=5)
    # v3.0: Fixed media_content_type — 'spotify://playlist' is required for Sonos integration
    http.post(f'{HA_URL}/api/services/media_player/play_media', headers=hh, json={'entity_id':SONOS,'media_content_id':f'spotify:playlist:{pid}','media_content_type':'spotify://playlist'}, timeout=5)
    time.sleep(1)
...
def index():
    ...
    return jsonify({'name':'Spotify Smart DJ','version':'3.0.0','slot':time_slot(),'weather':get_weather(),'kids_mode':kids_mode,'moods':moods,'total_plays':stats['total_plays'],'tv_paused':music_paused_by_tv})
...
def switch_speaker(entity):
    global SONOS
    # v3.0: Block Echo devices — they trigger the TV
    if entity in ECHO_ENTITIES:
        return jsonify({'error':'Echo devices trigger TV — use Sonos instead'}), 403
    # v3.0: Block bedroom entities without recent motion
    if entity in BEDROOM_ENTITIES and not is_bedroom_safe():
        return jsonify({'error':'Bedroom audio blocked — no recent bedroom motion detected'}), 403
    SONOS = entity
